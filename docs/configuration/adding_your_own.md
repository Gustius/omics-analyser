# nf-core/omics-analyser: Configuration for other clusters
...
### Docker
Docker is a great way to run nf-core/omics-analyser, as it manages all software installations and allows the pipeline to be run in an identical software environment across a range of systems.

Nextflow has [excellent integration](https://www.nextflow.io/docs/latest/docker.html) with Docker, and beyond installing the two tools, not much else is required - nextflow will automatically fetch the [nfcore/omics-analyser](https://hub.docker.com/r/nfcore/omics-analyser/) image that we have created and is hosted at dockerhub at run time.

To add docker support to your own config file, add the following:

```nextflow
docker.enabled = true
process.container = "nfcore/omics-analyser"
```
...
process.container = "shub://nf-core/omics-analyser"
```
...
```nextflow
singularity.enabled = true
process.container = "/path/to/nf-core-omics-analyser.simg"
```
